...
- **Channels.** GitHub marks prereleases, so stable-versus-early is a flag rather than a
  second feed — worth deciding whether it is offered at all before it is built.

**Accept when** a running copy notices a newer release, tells the user what changed, installs
it on quit, and relaunches into the same layout — and when a build made by `swift build`
offers no update at all.
