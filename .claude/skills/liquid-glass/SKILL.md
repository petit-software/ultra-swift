...
// ConcentricRectangle() — radius derived from the container, keeps nested shapes concentric
//   NOTE (verified against the macOS 26.5 SDK, Xcode 26.6): the concentric shape is the
//   `ConcentricRectangle` type, NOT `.rect(corner: .containerConcentric)` — that spelling
//   does not compile. Use `ConcentricRectangle()` or
//   `ConcentricRectangle(corners: .concentric(minimum: .fixed(8)))`, and set the container
//   radius with `.containerShape(.rect(cornerRadius:))`.
// .circle
```
